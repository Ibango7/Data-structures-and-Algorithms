...
public class MaxHeap<T extends Comparable<T>> extends Heap<T> {
    ...
    //Helper functions
    public void maxHeapifyUP(int nodeIndex) {
    ...
    public void maxHeapifyDown(int nodeIndex) {
    	...
    	while(!isLeaf(nodeIndex)) {
    		if(leftChild != -1 && rightChild != -1) {
    			largestChild = maxChild(leftChild, rightChild);
    		}
    		...
    		if(root.compareTo((T) mH[largestChild]) < 0) {
    			mH[nodeIndex] =  mH[largestChild];
    			mH[largestChild] = root;
    			nodeIndex = largestChild;
    			root = mH[nodeIndex];
